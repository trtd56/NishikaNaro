...
### Not Work
- robera loadできず（→https://huggingface.co/cl-tohoku　）
- 回帰、すべての確率を出さなければならない
...
class GCF:
    ...
    #MODEL_NAME = "cl-tohoku/roberta-base-japanese"
...
class NishikaMultiDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        d = {
            ...
            #"fav_novel_cnt_bin": torch.tensor(fav_novel_cnt_bin, dtype=torch.float),
        }
        return d
...
class NishikaMultiModel(nn.Module):
    ...
    def __init__(self, n_features, n_cate_dic):
        ...
        self.transformer_head = nn.Linear(self.config.hidden_size*GCF.N_USE_LAYER, self.config.hidden_size*GCF.N_USE_LAYER)
        # MLP
        n_emb = 64 #+ 8 + 4
        ...
        # head
        #self.classifier = nn.Linear(self.config.hidden_size*GCF.N_USE_LAYER+GCF.MLP_HIDDEN, 5)
        ...
        # reinit_layers
        if GCF.REINIT_LAYERS > 0:
            for layer in self.transformer_model.encoder.layer[-GCF.REINIT_LAYERS:]:
    ...
    def forward(self, X_cate, input_ids, attention_mask, token_type_ids, y=None):
        ...
        #logits_1 = self.classifier_1(cat_h)
        #logits_2 = self.classifier_2(cat_h)
    ...
    def loss_fn(self, y_pred, y_true):
        ...
        #loss = torch.sqrt(nn.MSELoss()(y_pred, y_true))
...
drop_features = ['upload_year', 'upload_month', 'userid_over5', 'hour', 'day_of_week']
...
for fold, (train_index, valid_index) in enumerate(skf):
    ...
    #optimizer = AdamW(optimizer_grouped_parameters, lr=GCF.MLP_LR, weight_decay=GCF.WEIGHT_DECAY)
