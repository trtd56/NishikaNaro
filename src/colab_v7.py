...
class NishikaMultiModel(nn.Module):
    ...
    def __init__(self, n_features, n_cate_dic):
        ...
        self.mlp1 = nn.Sequential(
            ...
            nn.Linear(GCF.AE_HIDDEN+GCF.N_EMBED, GCF.MLP_HIDDEN),
            #nn.Linear(GCF.AE_HIDDEN+GCF.N_EMBED+n_features, GCF.MLP_HIDDEN),
        ...
        #self.head_mlp_2 = nn.Linear(self.config.hidden_size*GCF.N_USE_LAYER+GCF.MLP_HIDDEN, self.config.hidden_size*GCF.N_USE_LAYER+GCF.MLP_HIDDEN)
        ...
        #self._init_weights(self.head_mlp_2)
    ...
    def forward(self, X_cate, input_ids, attention_mask, token_type_ids=None, y=None):
        ...
        #avg_pool = outputs["hidden_states"][-1].mean(1)
        #max_pool, _ = outputs["hidden_states"][-1].max(1)
        #sequence_output = torch.cat([avg_pool, max_pool], dim=1)
        ...
        ae1 = self.ae1(X_cate[:, :-1])
        ae2 = self.ae2(ae1)
        #h = torch.cat([X_cate[:, :-1], ae1, e], dim=1)
        h = torch.cat([ae1, e], dim=1)
        ...
        if y is not None:
            ...
            ae_loss = torch.nn.MSELoss()(ae2, X_cate[:, :-1])
            loss += ae_loss
        else:
            loss = None
...
'''
# StandardScaler
X = np.vstack([X_cate.values[:, :-1], X_cate_test.values[:, :-1]])
#transformer = StandardScaler()
transformer = RobustScaler()
X_scaled = transformer.fit_transform(X)
_X_cate = pd.DataFrame(X_scaled[:len(train_df), :], columns=X_cate.columns[:-1])
_X_cate_test = pd.DataFrame(X_scaled[len(train_df):, :], columns=X_cate.columns[:-1])
_X_cate['userid_only_test'] = X_cate['userid_only_test']
_X_cate_test['userid_only_test'] = X_cate_test['userid_only_test']
X_cate, X_cate_test = _X_cate, _X_cate_test
del _X_cate, _X_cate_test
'''

X = pd.concat([X_cate, X_cate_test]).reset_index(drop=True)
std_cate = []
for c in X.columns:
    if X[c].std() > 1 or X[c].max() - X[c].min() > 2:
        std_cate.append(c)
transformer = StandardScaler()
X_scaled = transformer.fit_transform(X[std_cate])
X[std_cate] = X_scaled
_X_cate = pd.DataFrame(X.loc[:len(train_df)], columns=X_cate.columns)
_X_cate_test = pd.DataFrame(X.loc[len(train_df):], columns=X_cate.columns)
X_cate, X_cate_test = _X_cate, _X_cate_test
del _X_cate, _X_cate_test, X
...
|108|0.7431||LGBM|
|109|0.7438||CatBoost|
|110|X||feat_all_norm, 0が多くなるので結果がNoneになる|
|111|X||lr=1e-3|
|112|0.7224|0.6648|auto encoderっぽく(113 cls tokenになっちゃってる)|
|114|0.7211|0.6658|cls token|
|115|0.7236|0.6664|2 layer|
|115|0.7544||XGBoost|
|116|0.7063||stdの大きい特徴量をnorm, fold-0まで, 良さそう|
|117|0.7126||avg&max pool, fold-0まで, 微妙|
|118|0.7103||avgのみ, fold-0まで|
|119|0.7116||skip conn, fold-0|
|120|0.7096||genre tag pred, fold-0|
|121|0.7148||headのmlp x 2, fold-0|
|122|0.7140||head 1|
|123|X||key word target|
|124|||storyの長さや改行を特徴量に入れる|
|125|||fix target with pseudo|
...
for _, row in tqdm(train_df.iterrows(), total=len(train_df)):
    ...
    with torch.no_grad():
        out = transformer_model(
            input_ids=torch.tensor(tok['input_ids'], dtype=torch.long).unsqueeze(0).to(device),
            attention_mask=torch.tensor(tok['attention_mask'], dtype=torch.long).unsqueeze(0).to(device),
            token_type_ids=torch.tensor(tok['token_type_ids'], dtype=torch.long).unsqueeze(0).to(device)
        )
    vec = out['last_hidden_state'][0].mean(0).cpu()
    vecs.append(vec)

np.save(f"{GCF.FEATURES_PATH}/bert_vector_train", torch.stack(vecs).numpy())
...
np.save(f"{GCF.FEATURES_PATH}/bert_vector_test", torch.stack(vecs).numpy())
